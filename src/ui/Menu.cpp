...
void Menu::Draw(const TextRenderer &text_renderer, const float scale, const Color text_color,
  ...
  for (const auto &[text, action]: options) {
    constexpr float option_gap = 20.0f;
    const Vector2 text_size = text_renderer.Measure(text, scaled_option_size);
    const Rectangle bounds{x, option_y, text_size.x, text_size.y};
    const bool hovered = CheckCollisionPointRec(GetMousePosition(), bounds);

    text_renderer.Render(text, {x, option_y}, scaled_option_size, hovered ? hovered_text_color : text_color);
