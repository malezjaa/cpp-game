...
public:
  ...
  void Draw(const TextRenderer &text, const Textures &textures, const float scale) {
...
private:
  ...
  [[nodiscard]] static float SlideProgress(const float elapsed, const float display_duration) {
  ...
  [[nodiscard]] static std::pair<Color, Color> ColorsFor(const AlertLevel level) {
    switch (level) {
      case AlertLevel::Warning:
